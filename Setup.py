...
logging . error (f" Failed to remove { user . first_name or ’Unknown ’} (ID: { user .id }): {e}")
print (f" Failed to remove { user . first_name or ’Unknown ’}: Check error .log ")
...
print (f"{i}. { group . title } (ID: { group .id })")
try :
group_choice = int ( input ("\ nEnter the number of the group to clean : ")) - 1
